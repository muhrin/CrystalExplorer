...
)
{
	...
	cellGenerator->setMinAngles(60.0);
	cellGenerator->setMaxAngles(120.0);
	...
	jdouble * sizes = env->GetDoubleArrayElements(sizes_, NULL);
	jdouble * strengths = env->GetDoubleArrayElements(strengths_, NULL);
	...
	for(size_t i = 0; i < numAtoms; ++i)
	{
		...
		sigma(i, i) = sizes[i];
		epsilon(i, i) = strengths[i];
		speciesList.push_back(elements[i]);
	}

	env->ReleaseIntArrayElements(numbers_, numbers, 0);
	env->ReleaseDoubleArrayElements(sizes_, sizes, 0);
	env->ReleaseDoubleArrayElements(strengths_, strengths, 0);
	...
	optOptions.maxSteps.reset(1000);
	optimiser.setTolerance(1e-5);
